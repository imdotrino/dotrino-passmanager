...
| ✅ la extensión recuerda la cuenta | al entrar con contraseña, su DIRECCIÓN queda apuntada en ese navegador (no la contraseña ni las llaves). Cuando la sesión se cierra —«Salir», o se cerró el navegador en una cuenta de paso— sigue en el selector de perfiles como «sesión cerrada», y elegirla abre la pantalla de entrar con la dirección puesta. «Olvidar esta cuenta» la quita. Entrar es SOLO «Iniciar sesión» del botón de perfil: la lista de la bóveda no lo ofrece, y los accesos se crean desde la página del perfil (dueño, 2026-09-21) |
| **PENDIENTE** | destinatarios elegibles por entrada (`sealed-passwords.md`): elegir qué entradas recibe cada aparato |
| **PENDIENTE** | recordar la cuenta cerrada también en las demás apps del ecosistema (hoy solo en la extensión; en las apps el selector lo pinta `@dotrino/identity`) |
...
| ✅ la extensión | lo mismo, en su gestor. El WASM de OPAQUE corre en una **página sandbox** —la CSP se le abre solo a ella, no a toda la extensión, así que la extensión no declara `wasm-unsafe-eval` ni pide el permiso `offscreen`— y **atender abre una pestaña** (`src/vault-tab.html`): ahí vive el socket, y esa pestaña ES el interruptor |
| ✅ la extensión (probado) | `npm run test:opaque` levanta un Chrome de verdad, hace la vuelta entera de OPAQUE por el puente y mira que no salte ni una violación de CSP. Era lo único que no se podía comprobar leyendo el manifiesto: un iframe sandbox que no arranca **no se ve** |
| ✅ el CLI | `logins add <usuario> [nombre] [±permiso …]` — se pueden tener VARIOS por perfil y cada uno con sus permisos |
| ✅ la TUI del binario | la pestaña **Entrar** (←→), con las mismas acciones y el MISMO canal que `dotrino-vault logins` — una pieza, no dos, así que no se pueden desincronizar. Teclas del resto de la TUI: `n` nuevo · `c` contraseña · `x` cerrar sesiones · `u` quitar la espera · `v` quitar |
| ✅ `@dotrino/passmanager` (extensión) | la pantalla de ENTRAR (`manager.html#view=login`), a la que lleva «Iniciar sesión» del menú del botón de perfil. Corre en la PÁGINA —el OPAQUE está en el sandbox y un worker no sostiene un socket— y le pasa al núcleo lo que salió, para que lo adopte él |
| ✅ `@dotrino/vault` 0.69.0 | `loginWithPassword({ opaque })` y `closeLogin({ sign })`: en una extensión las piezas están repartidas y el pilar no puede dar por hecho que están en el mismo hilo |
| ✅ cuenta de paso en la extensión | `chrome.storage.session` (memoria, se vacía al cerrar el navegador) es el equivalente exacto del `sessionStorage` de una pestaña. Sin él, el worker se duerme a los 30 s y la cuenta se barría sola |
