...
| ✅ la bóveda DENTRO de la extensión | mismo formato. Se convierte sola, sin pedir nada, y **atiende aunque aún no tenga copia de recuperación** (dueño, 2026-09-24: *«esta bien la alerta pero no debe ser bloqueante»*): lo guardado se sella a los aparatos del acta, la falta se avisa en el popup y en el gestor, y al poner la contraseña (`manager.html#view=convert`) `SealedStore.addRecovery` le añade `#recovery` a todo lo anterior con la llave de la propia extensión. El almacén exige `#recovery` solo cuando la cuenta ya tiene copia (opción `recovery()`, estricta por defecto: el binario y `serve` no cambian). Aquí, y solo aquí, **la bóveda SÍ está entre los destinatarios**: no hay cartero y aparato, hay uno solo — excluirse dejaría una bóveda que solo se abre escribiendo la contraseña en cada campo, que no es más segura, es inusable |
