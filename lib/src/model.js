...
import { normalizeFields, entryFieldKeys } from './fields.js'
 ...
 * «rellenar el número de socio» sin sacar nada de la bóveda. Sin ella, la vista dice
 * `hasFields` y poco más, que es lo que había.
 */
export function publicView (entry, hint, open) {
  return {
    id: entry.id,
    type: entry.type,
    title: entry.title,
    ...
    updatedAt: entry.updatedAt,
    ...(hint ? { hint } : {}),
    ...(open ? { fieldKeys: entryFieldKeys(open) } : {}),
  }
}
 ...
 *
 * Su dato público más claro, **sin esconder caracteres** (dueño, 2026-08-28): el usuario;
 * si no tiene, el correo; y si tampoco, el primer campo que lleve. Un `d•••g` no le dice
 * nada a nadie — hay que poder reconocer la cuenta de un vistazo.
 *
