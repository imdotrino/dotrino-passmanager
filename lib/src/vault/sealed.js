...
import { profileKeys, siteIndex, passkeyIndex, valueDigest } from '../sealed/keys.js'
...
/** De la clave que usa la interfaz a la del sobre. Los campos libres llevan `f:`. */
const toSealedKey = (k) => (FIXED.includes(k) || k.startsWith('wa.') ? k : 'f:' + k)
...
export class SealedVault {
  ...
  async #openView (v) {
    ...
    // `fieldHashes` conserva el nombre que ya usa la interfaz para comparar. Lo que cambia
    // es de dónde salen: antes era un HMAC con un nonce de la respuesta y ahora son los
    // resúmenes guardados (§2.6), que es lo que permite compararlos sin abrir nada.
    return { ...abierta, fieldHashes: abierta.digests }
  }
  ...
  async get (id, opts = {}) {
    const keys = Array.isArray(opts.keys) ? opts.keys.map(toSealedKey) : null
  ...
  async patch (id, changes = {}) {
    ...
    const values = {}
    const drop = []
    for (const k of FIXED) if (typeof changes[k] === 'string') values[k] = changes[k]
    ...
    for (const f of campos) {
      const k = fieldKey(f)
      if (typeof f.value === 'string' && f.value) values['f:' + k] = JSON.stringify({ label: f.label, value: f.value, kind: f.kind })
      // La marca de privado es de la vista: cambiarla no toca ningún sobre.
    ...
    const parche = await buildSealedPatch({
      id, view: vista, values, drop, priv: vista.privateKeys.map((k) => toSealedKey(k)),
