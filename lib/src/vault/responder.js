...
export class VaultResponder {
  ...
  async #handle (msg, fromPubkey) {
    ...
    try {
      ...
      // Lo que la bóveda marca `alwaysAsk` se pregunta SIEMPRE, aunque el aparato ya
      // esté aprobado. Es la salida para la del banco.
      const entry = msg.op === 'get' && payload.id
        ? await this.vault.get(payload.id).catch(() => null)
        : null
      const siempre = !!entry?.alwaysAsk

      if (this.needsApproval(msg.op, payload, fromPubkey)) {
        const ok = siempre
          ? await this.approve({ op: msg.op, payload, pubkey: fromPubkey, entry })
          : await this.#approveDevice(fromPubkey, msg.op, payload)
        if (!ok) {
