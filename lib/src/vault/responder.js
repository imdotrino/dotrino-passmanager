...
export class VaultResponder {
  ...
  async #handle (payload, fromPubkey) {
    // **Nada sin cifrar entra.** No es una comprobación de más: sin ella bastaría con
    // mandar la petición en claro para saltarse el sellado entero, y el proxio vería
    // qué se pide. Un mensaje en claro con la forma correcta es exactamente lo que
    // haría quien quisiera eso, así que se rechaza y se anota.
    if (!isSealed(payload)) {
      if (isRequest(payload)) {
        this.onRequest({ op: payload.op, from: fromPubkey, ts: Date.now(), outcome: 'unsealed' })
      }
      return
    }
    ...
    try {
      ...
      // Sellado a otro: ni se contesta. Contestar diría que aquí hay una bóveda.
      return
    }
