...
import { openEntry, sealEntry, publicView, maskUsername } from '../model.js'
...
export class LocalVault {
  ...
  async find (url) {
    ...
    // El hint obliga a abrir el usuario: por eso lo calcula quien tiene la CEK, y no
    // quien pregunta.
    return Promise.all(hits.map(async ({ entry }) => {
      const open = await openEntry(this.key, entry)
      return publicView(entry, maskUsername(open.username))
    }))
  }
