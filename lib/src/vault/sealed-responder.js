...
export class SealedResponder {
  ...
  start () {
    ...
    this._handler = async (from, msg, meta) => {
      ...
      const pubkey = this.client?.pubkeyOfToken?.(from) || meta?.pubkey || from
      await this.handle({ from, pubkey, msg: p, sealed: meta?.sealed !== false })
    }
  ...
  async handle ({ from, pubkey, msg, sealed = true }) {
    ...
    if (!sealed) { anota('unsealed'); return this.#send(from, replyError(rid, CODES.NOT_ALLOWED, 'unsealed request')) }
    if (!SEALED_OPS.includes(op)) { anota('unknown-op'); return this.#send(from, replyError(rid, CODES.NOT_ALLOWED, 'operación no permitida: ' + op)) }
    if (!this.isAllowed(pubkey)) { anota('denied'); return this.#send(from, replyError(rid, CODES.NOT_ALLOWED, 'este aparato no puede pedir contraseñas')) }

    try {
      if (await this.#needsApproval(op, payload, pubkey)) {
        const ok = await this.gate.ask({ op, payload, pubkey })
        if (!ok) { anota('rejected'); return this.#send(from, replyError(rid, CODES.DENIED, 'no autorizado desde el aparato de confianza')) }
      }
      const result = await this.#run(op, payload, pubkey)
      anota('ok')
      return this.#send(from, reply(rid, result))
    } catch (e) {
      anota('error', { code: e?.code })
      return this.#send(from, replyError(rid, e?.code || CODES.UNKNOWN, e?.message || 'error'))
    }
  }
  ...
  async #run (op, payload = {}, pubkey) {
    switch (op) {
      ...
      default: throw Object.assign(new Error('operación no permitida: ' + op), { code: CODES.NOT_ALLOWED })
    }
  }

  #send (to, msg) {
    const encPub = this.encPubOf(this.client?.pubkeyOfToken?.(to) || to)
