...
// hace SIN sesgo: `% alfabeto.length` reparte de más los primeros caracteres cuando el
// alfabeto no divide a 256, y en un generador de contraseñas eso es exactamente lo que
...
function pick (alfabeto) {
  return alfabeto[randomBelow(alfabeto.length)]
}
...
export function generatePassword ({
  ...
  const grupos = [ambiguous ? MINUS + 'l' : MINUS]
  if (upper) grupos.push(ambiguous ? MAYUS + 'IO' : MAYUS)
  if (digits) grupos.push(ambiguous ? DIGITOS + '01' : DIGITOS)
  if (symbols) grupos.push(SIMBOLOS)

  const alfabeto = grupos.join('')
  const min = Math.max(grupos.length, 4)
  const largo = Math.max(min, Math.min(256, Math.floor(length) || 20))

  // Uno de cada grupo pedido, para que «con números» signifique que HAY un número y no
  // que podría haberlo. El resto, del alfabeto entero.
  const chars = grupos.map(g => pick(g))
  while (chars.length < largo) chars.push(pick(alfabeto))
