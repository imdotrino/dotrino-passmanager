...
export async function sealEntry ({ plain, main, passkeys = null, keys }) {
  ...
  for (const f of campos) {
    if (!f?.value) continue
    // LA MARCA DE PRIVADO NO VA EN EL SOBRE, VA EN LA VISTA (`privateKeys`). Es metadato de
    // permiso, no parte del valor — y ponerla dentro obligaba a abrir el campo para
    // cambiarla, que es justo lo que el formato viene a evitar: marcar un teléfono como
    // privado no puede exigir sacar el teléfono de la bóveda.
    const { private: _p, ...guardable } = f
    fields['f:' + fieldKey(f)] = await sealV(main.cek, main.gen, JSON.stringify(guardable))
  }
  ...
  return {
    ...
    fields,
    digests,
    // QUÉ CAMPOS SON PRIVADOS, EN CLARO Y SOLO SUS NOMBRES. Es lo único de la vista que la
    // bóveda necesita ver, y necesita verlo porque la aprobación es SUYA: sin esto no
    // podría distinguir «rellena mi nombre» de «dame la contraseña», y acabaría pidiendo un
    // dedo encima para todo —que es como se enseña a decir que sí sin mirar—.
    //
    // El precio, dicho claro: una copia del disco de la bóveda sabe que una entrada tiene un
    // campo privado llamado `id-number`. No sabe de qué entrada es (el título va sellado),
    // ni su valor, ni en qué sitio se usa. Y es lo mismo que ya revela cualquier petición,
    // porque la clave viaja en ella.
    priv: [...privateKeysOf(abierta)].map((k) => (FIXED.includes(k) || k.startsWith('wa.') ? k : 'f:' + k))
  }
}
...
export async function openFields ({ sealed, envelopes, open, view = null }) {
  ...
  const libres = []
  const privados = new Set(view?.privateKeys || [])
  ...
  for (const [k, envelope] of Object.entries(envelopes || {})) {
    ...
    if (k.startsWith('f:')) {
      // La marca de privado vuelve desde la vista, que es donde vive.
      try { const f = JSON.parse(valor); libres.push(privados.has(k.slice(2)) ? { ...f, private: true } : f) } catch (_) {}
      continue
    }
