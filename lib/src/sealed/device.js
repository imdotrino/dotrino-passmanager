 ...
 *   `values`       `{ clave: valor }` solo lo que cambia
...
export async function buildSealedPatch ({ id, view, values = {}, drop = [], priv = null, idx = null, wa, keys, recipients, author } = {}) {
  ...
  const fields = {}
  const digests = {}
  for (const [k, valor] of Object.entries(values)) {
    const conQue = k === PASSKEY_FIELD ? cekPk : cek
    fields[k] = await encryptWithCek({ cek: conQue, gen: 0, plaintext: String(valor) })
    // El resumen es del VALOR, con el id de la entrada dentro (§2.6). La privada de una
    // passkey no se compara nunca: no se teclea, así que no hay nada que cotejar.
    if (k !== PASSKEY_FIELD) digests[k] = await valueDigest(keys.kcmp, id, k.replace(/^f:/, ''), String(valor))
  }
