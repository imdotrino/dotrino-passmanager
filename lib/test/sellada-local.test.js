...
test('la vuelta entera: guardar, listar, abrir, buscar y quitar', async () => {
  ...
  // La forma es la de `LocalVault.sites()` y tiene que serlo: es lo que pinta el gestor.
  assert.deepEqual(await vault.sites(), [{ site: 'banco.example', count: 1 }, { site: '', count: 1 }])
...
/**
 * Y LA FUSIÓN DE UN PARCHE, que es la misma que `LocalVault` y tiene que serlo.
 *
 * No es un `{...a, ...b}`: **un campo sin `value` dice «déjale el que tenga»**, no
 * «bórralo». Marcar uno como privado llega exactamente así —solo su etiqueta y la marca—,
 * y fusionarlo a lo bruto le borraba el valor. Costó encontrarlo porque no falla: guarda
 * tan contento, y el campo ya no está.
 */
test('marcar un campo como privado NO le borra el valor', async () => {
  const { vault } = await montar()
  await vault.put({
    id: 'e1',
    type: 'data',
    title: 'Datos',
    sites: [],
    fields: JSON.stringify([{ label: 'Correo', value: 'a@b.c' }, { label: 'Número de socio', value: 'SOC-1' }])
  })

  await vault.patch('e1', { fields: [{ label: 'Número de socio', private: true }] })

  const e = await vault.get('e1')
  const campos = JSON.parse(e.fields)
  assert.equal(campos.length, 2, 'los dos campos siguen ahí')
  assert.equal(campos.find((f) => f.label === 'Número de socio')?.value, 'SOC-1', 'y con su valor')
  assert.equal(campos.find((f) => f.label === 'Número de socio')?.private, true)
  assert.equal(campos.find((f) => f.label === 'Correo')?.value, 'a@b.c', 'el otro ni se toca')
})

test('quitar un campo lo quita, y dejar uno vacío también', async () => {
  const { vault } = await montar()
  await vault.put({
    id: 'e1', type: 'data', title: 'Datos', sites: [],
    fields: JSON.stringify([{ label: 'Correo', value: 'a@b.c' }, { label: 'Teléfono', value: '099' }])
  })
  await vault.patch('e1', { fields: [{ label: 'Teléfono', value: '' }] })
  const campos = JSON.parse((await vault.get('e1')).fields)
  assert.deepEqual(campos.map((f) => f.label), ['Correo'], 'un campo sin valor no es un campo')
})
