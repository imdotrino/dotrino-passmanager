...
test('si el teléfono dice que no, no sale', async () => {
  ...
  await vault.put(CUENTA)
  // Por CÓDIGO, no por el texto: «el humano dijo que no» (`not-approved`) se arregla
  // volviendo a pedir, y «este aparato no puede» (`denied`) dando permiso. Antes llegaba
  // `denied` y la prueba no lo notaba porque miraba la frase.
  await assert.rejects(() => vault.get('e1', { keys: ['secret'] }), (e) => e.code === 'not-approved')
...
test('marcar como privado un campo PÚBLICO no despierta al teléfono', async () => {
  const { vault, pedidos } = await montar()
  await vault.put(CUENTA)
  pedidos.length = 0

  // La marca va DENTRO del sobre, así que cambiarla reescribe el campo y hay que leerlo
  // antes. Leer uno público no cuesta aprobación — que es justo por lo que se puede hacer
  // desde el gestor sin fricción. Quitarle la marca a uno privado sí la costaría, y está
  // bien: vas a exponerlo.
  await vault.patch('e1', { fields: [{ label: 'socio', kind: 'id-number', private: true }] })
  assert.equal(pedidos.length, 0, 'marcar un campo público pidió un dedo encima')
...
test('la bóveda no ve el NOMBRE de un campo libre: lo guarda por huella', async () => {
  const { vault, store } = await montar()
  await vault.put({ ...CUENTA, fields: [{ label: 'Cédula', value: '1700123456', kind: 'id-number', private: true }] })

  const crudo = JSON.stringify(await store.store.get('passmanager/sealed/v2'))
  for (const nombre of ['Cédula', 'id-number', '1700123456']) {
    assert.ok(!crudo.includes(nombre), `«${nombre}» está en claro en el disco de la bóveda`)
  }
  // Y aun así la bóveda sabe que ESA clave es privada, porque su índice va con la misma
  // huella: pedirla pide aprobación.
  const { pedidos } = await montar()
