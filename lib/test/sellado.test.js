...
import { openWrap, decryptWithCek, wrapForMember } from '@dotrino/identity/content'
...
async function device (nombre) {
  ...
  return {
    ...
    // Lo mismo que `rewrapFor` de `@dotrino/identity`: abre la suya y envuelve para otro.
...
test('PAGAR LA DEUDA sin la frase: un miembro con envoltura completa al que entró después', async () => {
  const uno = await device('extensión'); const tarde = await device('el equipo prestado')
  const rec = await recovery()
  const mem = new Map()
  const store = { async get (k) { return mem.get(k) }, async set (k, v) { mem.set(k, v) } }
  // El acta CAMBIA: al principio solo está la extensión; luego entra otro aparato.
  const miembros = [uno]
  const sealed = new SealedStore(store, {
    recipients: () => miembros.map((d) => ({ pub: d.pub, encPub: d.encPub })),
    verifyAuthor: async ({ body, author }) => {
      const quien = miembros.find((d) => d.pub === author.pub)
      if (!quien) return false
      return crypto.subtle.verify(SIGN, quien.verifyKey,
        Uint8Array.from(Buffer.from(author.sig, 'base64')), new TextEncoder().encode(canon(body)))
    }
  })
  const recipients = () => ({ recoveryPub: rec.encPub, main: miembros.map((d) => ({ pub: d.pub, encPub: d.encPub })), passkeys: [] })
  await conEntrada({ sealed, recipients: recipients() }, uno, { ...CUENTA, totp: '' })

  miembros.push(tarde)
  assert.deepEqual((await sealed.incompleteMembers()).map((m) => m.pub), [tarde.pub], 'el recién llegado debería deber')
  const antes = await sealed.get('e1', { pub: tarde.pub }).catch((e) => e)
  assert.equal(antes.code, 'not-yours', 'sin reenvolver, el que entró tarde no abre nada')

  const r = await sealed.rewrapAll({ holder: { pub: uno.pub, rewrapFor: uno.rewrapFor } })
  assert.ok(r.wrapped >= 2 && !r.failed.length, `debería haber reenvuelto el perfil y la entrada: ${JSON.stringify(r)}`)
  assert.deepEqual(await sealed.incompleteMembers(), [])

  const got = await sealed.get('e1', { pub: tarde.pub })
  const abierta = await openSealedEntry({ got, openSealed: tarde.openSealed })
  assert.equal(abierta.secret, CUENTA.secret, 'lo que guardó la extensión lo abre el que entró después')
})

test('rewrapAll sin frase ni envoltura propia no se inventa nada: lo dice', async () => {
  const uno = await device('extensión'); const tarde = await device('otro'); const ajeno = await device('sin envoltura')
  const v = await boveda({ soloPasswords: [uno] })
  await conEntrada(v, uno, { ...CUENTA, totp: '' })
  // Un «holder» que no está en la generación no puede reenvolver: falla por generación.
  const sealed2 = new SealedStore({ get: async (k) => v.mem.get(k), set: async (k, x) => v.mem.set(k, x) }, {
    recipients: () => [uno, tarde].map((d) => ({ pub: d.pub, encPub: d.encPub })),
    verifyAuthor: async () => false
  })
  const r = await sealed2.rewrapAll({ holder: { pub: ajeno.pub, rewrapFor: ajeno.rewrapFor } })
  assert.equal(r.wrapped, 0)
  assert.ok(r.failed.length >= 1 && r.failed.every((f) => /not wrapped for the holder/.test(f.error)))
  await assert.rejects(() => sealed2.rewrapAll({}), /openRecovery.*holder/)
})

test('SIN COPIA DE RECUPERACIÓN la bóveda funciona igual, y la copia se añade después', async () => {
  const uno = await device('extensión')
  const rec = await recovery()
  const mem = new Map()
  const store = { async get (k) { return mem.get(k) }, async set (k, v) { mem.set(k, v) } }
  let tieneCopia = false
  const sealed = new SealedStore(store, {
    recipients: () => [{ pub: uno.pub, encPub: uno.encPub }],
    recovery: async () => tieneCopia,
    verifyAuthor: async ({ body, author }) => author.pub === uno.pub && crypto.subtle.verify(SIGN, uno.verifyKey,
      Uint8Array.from(Buffer.from(author.sig, 'base64')), new TextEncoder().encode(canon(body)))
  })
  const recipients = { recoveryPub: null, main: [{ pub: uno.pub, encPub: uno.encPub }], passkeys: [] }
  await conEntrada({ sealed, recipients }, uno, { ...CUENTA, totp: '' })

  const got = await sealed.get('e1', { pub: uno.pub })
  assert.equal((await openSealedEntry({ got, openSealed: uno.openSealed })).secret, CUENTA.secret)
  assert.ok(mem.get(KEY).keyring.every((g) => !g.wraps[RECOVERY]), 'sin copia no debería haber #recovery')

  // Llega la copia: a partir de ahí, lo nuevo sin #recovery no entra...
  tieneCopia = true
  const sin = await buildSealedEntry({ plain: { ...CUENTA, id: 'e2', totp: '' }, keys: await profileKeys(await conEntradaKeys(sealed, uno)), recipients, author: uno.author })
  const e = await sealed.putSealed(sin).catch((x) => x)
  assert.equal(e.code, 'recovery-missing')
  // ...y lo de antes la recibe con la llave de la extensión, sin pedir la frase.
  const r = await sealed.addRecovery({ recoveryPub: rec.encPub, holder: { pub: uno.pub, rewrapFor: uno.rewrapFor } })
  assert.ok(r.wrapped >= 2 && !r.failed.length, JSON.stringify(r))
  const g = mem.get(KEY).keyring.find((x) => x.gen === mem.get(KEY).entries[0].fields.secret.gen)
  const porFrase = await rec.openSealed({ wrap: g.wraps[RECOVERY], envelope: mem.get(KEY).entries[0].fields.secret })
  assert.ok(porFrase.includes(CUENTA.secret), 'la copia de recuperación debería abrir lo guardado antes de crearla')
})

/** La llave del perfil, abierta por el aparato: para construir una entrada más. */
async function conEntradaKeys (sealed, d) {
  const { envelope, wrap } = await sealed.profile({ pub: d.pub })
  return d.openSealed({ wrap, envelope })
}

test('por defecto el almacén EXIGE la copia de recuperación: lo estricto es lo que no se pide', async () => {
  const uno = await device('extensión')
  const v = await boveda({ soloPasswords: [uno] })
  const pk = await buildProfileKey({ recipients: { ...v.recipients, recoveryPub: null } })
  const e = await v.sealed.setProfile({ envelope: pk.envelope, wraps: pk.wraps }).catch((x) => x)
  assert.equal(e.code, 'recovery-missing')
  await assert.rejects(() => buildProfileKey({ recipients: undefined }), /ask the vault for the recipients/)
})
