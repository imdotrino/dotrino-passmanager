...
test('generar: el reparto de caracteres no está sesgado', () => {
  // 4000 caracteres de un alfabeto de 8 dígitos: si hubiera sesgo por `% length`,
  // los primeros saldrían claramente más. Se admite holgura, pero no el doble.
  const cuenta = new Map()
  for (let i = 0; i < 500; i++) {
    for (const c of generatePassword({ length: 8, upper: false, symbols: false })) {
      cuenta.set(c, (cuenta.get(c) || 0) + 1)
    }
  }
  const valores = [...cuenta.values()]
  assert.ok(Math.max(...valores) < Math.min(...valores) * 2, 'reparto sesgado')
})
